...
public final class RSACipher extends CipherSpi {
    ...
    private es.mityc.javasign.pkstore.mscapi.mityc.Key publicKey;
    // the private key, if we were initialized using a private key
    private es.mityc.javasign.pkstore.mscapi.mityc.Key privateKey;
    ...
    private void init(int opmode, Key key) throws InvalidKeyException {
        ...
        if (!(key instanceof es.mityc.javasign.pkstore.mscapi.mityc.Key)) {
            throw new InvalidKeyException("Unsupported key type: " + key);
        }

        if (key instanceof PublicKey) {
            mode = encrypt ? MODE_ENCRYPT : MODE_VERIFY;
            publicKey = (es.mityc.javasign.pkstore.mscapi.mityc.Key)key;
            privateKey = null;
            outputSize = publicKey.bitLength() / 8;
        } else if (key instanceof PrivateKey) {
            mode = encrypt ? MODE_SIGN : MODE_DECRYPT;
            privateKey = (es.mityc.javasign.pkstore.mscapi.mityc.Key)key;
            publicKey = null;
    ...
    protected int engineGetKeySize(Key key) throws InvalidKeyException {
        ...
        if (key instanceof es.mityc.javasign.pkstore.mscapi.mityc.Key) {
            return ((es.mityc.javasign.pkstore.mscapi.mityc.Key) key).bitLength();
        } else {
            throw new InvalidKeyException("Unsupported key type: " + key);
        }
    }
